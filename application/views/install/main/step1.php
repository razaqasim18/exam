...
$constants_file_write_perm = is_writable('application/config/constants.php');
...
if ($constants_file_write_perm == false || $db_file_write_perm == false || $routes_file_write_perm == false || $curl_enabled == false) {
  $valid = false;
} else {
  $valid = true;
}
  ...
  <div class="col-md-8 col-md-offset-2">
    <div class="panel panel-default" data-collapsed="0"
      style="border-color: #dedede;">
        ...
        <p style="font-size: 14px;">
          <?php if ($constants_file_write_perm == true) { ?>
            <i class="fa fa-check" style="color: #5ac52d;"></i>
          <?php } else { ?>
            <i class="fa fa-times" style="color: #f12828"></i>
          <?php } ?>
          <strong>application/config/constants.php </strong>: file has write permission
        </p>
        ...
        <?php if ($valid == true) { ?>
          <p>
            <?php if ($_SERVER['SERVER_NAME'] == 'localhost' || $_SERVER['SERVER_NAME'] == '127.0.0.1') { ?>
              <a href="<?php echo base_url();?>install/step3" class="btn btn-info">
                Continue
              </a>
            <?php } else { ?>
