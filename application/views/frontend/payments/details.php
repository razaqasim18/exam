        ...
        <div class="col-xs-12 col-sm-9 col-md-9">
            ...
            <div class="notice-details">
                <div id="printableArea">
                    ...
                    $bill_info .='<tr>';
                    $bill_info .='<td style="border: 0px;text-align: left;" width="50%" class="payment-to" > <b>'.getlang('site_payment_to', 'data').'</b> <br />';
                    ...
                    if($payment_alias != "offline"){
                        ...
                        $field_status  = fieldBuilder('select', 'status', $status_filter, getlang('site_status', 'data'), '');

                        // Filter Comment
                        $comment_area = '<textarea cols="" rows="" name="comment" class="form-control input-sm pull-right" >'.$comment.'</textarea>';
                        $field_comment  = fieldBuilder('select', 'comment', $comment_area, getlang('site_commnet', 'data'), '');
                        
                        $review ='
                            <form method="post" style="margin-top: 30px;" action="'.base_url().'user/payments/review/'.$id.'" class="form-horizontal ">
                            '.$field_status.'
                            '.$field_comment.'
                            <input type="hidden"  name="review_by" value="'.$this->uid.'" />
                            <input type="hidden" id="csrf" name="'.$token_name.'" value="'.$token_hash.'" />
                            <input type="submit"  class="btn btn-small btn-primary " style="float: right;" value="'.getlang('site_btn_submit', 'data').'" />
                            </form>
                        ';
                        echo $review;
                    }
