    ...
    <div class="row">
            <div class="col-xs-12">
                ...
                <div id="printableArea">
                    ...
                    $header .='<span> '.getlang('issue', 'sys_data').': '.$issue_date.'</span> <br />';
                    $header .='<span>'.getlang('status', 'sys_data').': '.$paid_status.'</span> <br />';
                    $header .='<span>'.getlang('paid_by', 'sys_data').': '.$paid_by.'</span> <br />';
                    $header .='</td>';
                    $header .='</tr>';   
                    $header .='</table>';
                    ...
                    $bill_info .='<tr>';
                    $bill_info .='<td style="border: 0px;text-align: left;" width="50%" class="payment-to" > <b>'.getlang('payment_to', 'sys_data').'</b> <br />';
                    $bill_info .='<span>'.$schools_name.'</span> <br />';
                    $bill_info .='<span>'.$schools_address.'</span> <br />';
                    $bill_info .='<span>'.$schools_phone.'</span> <br />';
                    $bill_info .='</td>';
                    $bill_info .='<td style="border: 0px;text-align: right;" width="50%" class="bill-to" > <b>'.getlang('invoice_to', 'sys_data').'</b> <br />';
                    ...
                    if($payment_alias != "offline"){
                        ...
                        // Filter Filter
                        $status_filter = getPaymentStatus('status', $status, 'form-control input-sm pull-right', '');
                        $field_status  = fieldBuilder('select', 'status', $status_filter, 'Status', '');

                        // Filter Comment
                        $comment_area = '<textarea cols="" rows="" name="comment" class="form-control input-sm pull-right" >'.$comment.'</textarea>';
                        $field_comment  = fieldBuilder('select', 'comment', $comment_area, 'Comment', '');
                        
                        $review ='
                            <form method="post" style="margin-top: 30px;" action="'.base_url().ADMIN_ALIAS.'/payments/review/'.$id.'" class="form-horizontal ">
                            '.$field_status.'
                            '.$field_comment.'
                            <input type="hidden"  name="review_by" value="'.$uid.'" />
                            <input type="hidden" id="csrf" name="'.$token_name.'" value="'.$token_hash.'" />
                            <input type="submit"  class="btn btn-small btn-primary " style="float: right;" value="'.getlang('btn_submit', 'sys_data').'" />
                            </form>
                        ';
                        echo $review;
                    }
